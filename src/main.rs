...
fn main() {
    ...
    match choice {
        ...
        Choices::EcoGame => {
            println!("Have fun repeating words boi boi. :3");
            println!("");
            print_words_given_to_it();
        }
    }
}
...
fn return_right_option() -> Choices {
    ...
    //match.
    let choice_num: i32 = choice_string
        .trim()
        .parse()
...
fn print_words_given_to_it() {
    println!("Give a word to repeat bish.");
    let repeat_words: String = get_input!();
    println!("{}", repeat_words);
}
